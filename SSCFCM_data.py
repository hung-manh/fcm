import yaml
import time
from models.SSCFCM import SSCfcm
...
if __name__ == "__main__":
    ...
    if DATA_ID in TEST_CASES:
        ...
        # --------------------------------
        _start_time = time.time()
        sscfcm = SSCfcm(m, epsilon, maxiter)
        C_list = np.array([5, 6, 7])
        datas, labeled, u_bar = create_membership_label(_dt['X'], _dt['Y'], n_sites= 3, ratio=0.001, C=C_list)
        U, V, step = sscfcm.sscfcmeans(datas, u_bar, C_list, seed=42)
        metrics = []
