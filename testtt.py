...
# ------------------------------------------
    ...
    # ------------------------------------------
    # Ảnh màu nối tiếp
    data, image_data_shape = image2data(input_image_label_path) 
    data1, image_data_shape1 = image2data(input_image_path)
    data = np.where(data > 20, 1, 0) # Đang thấy hơi vấn đề ? 
    data = data[:, 0] # Đây là nhãn của bức ảnh, shape = (n, 1)
    data, labeled, u_bar = create_membership_for_semi_supervised_learning_image(data1, data, ratio=0.8, C=C) # Truyền vào 2 cái, data thật và nhãn
    _start_time = time.time()
    
    
    dfcm = SSfcm(m, epsilon, maxiter)
    U3, V3, step = dfcm.sscmeans(data, u_bar, C, seed)
